...
export default function CardMusica({ musicas }) {
  ...
  return (
      <div className="flex flex-col p-3 w-56">
        <div className="relative">
          ...
          <img className="rounded-lg" src={musicas.cover_medium}></img>
        </div>
        <h4 className="text-xl my-2 line-clamp-1 w-fit">{musicas.title}</h4>

        <div className="flex justify-center items-center">
          <StarIcon className="text-ambar-400" />
          <span className="text-lg">{musicas && musicas.artist && musicas.artist.name}</span>
        </div>
